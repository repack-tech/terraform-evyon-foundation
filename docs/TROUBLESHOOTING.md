...
```
state snapshot was created by Terraform v0.x.x, which is newer than current v0.14.12; upgrade to Terraform v0.x.x or greater to work with this state
```

**Cause:**

The manual deploy step for the shared environment in [3-networks](../3-networks#deploying-with-cloud-build) was executed with a Terraform version newer than version v0.14.12 used in the **Foundation Pipeline**.
...
You will need to re-run the deploy of the 3-networks shared environment with Terraform v0.14.12.
...
- Install Terraform v0.14.12.
- Re-run the manual deploy of 3-networks shared environment using Terraform v0.14.12.
...
- Edit the module `cloudbuild_bootstrap` in the Terraform [main.tf](../0-bootstrap/main.tf) file:
  - Upgrade `terraform_version` from `"0.14.12"` to `"0.x.x"`
